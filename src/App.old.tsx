...
function calcScore(answers: Answers): ScoreResult {
  ...
  const willingAvg =
    willingQs.reduce((s, id) => s + ((answers[id] as number) || 3), 0) /
    willingQs.length;
  const controlAvg =
    controlQs.reduce((s, id) => s + ((answers[id] as number) || 3), 0) /
    controlQs.length;
  const immersion = Math.round(((6 - controlAvg) / 4) * 100);
  const willing = Math.round(((willingAvg - 1) / 4) * 100);
  const trigBonus = Math.min(
    ((answers["q8"] as number[]) || []).length * 8,
    30
  );
  const relapse = answers["q5"] === "yes" ? 10 : 0;
  const impactBonus = Math.min(
    (((answers["q10"] as number) || 0) / 10) * 15,
    15
  );
  return {
    immersionScore: Math.min(
      100,
      immersion + trigBonus + relapse + impactBonus
    ),
    willingScore: willing,
  };
}
...
function getTier(immersionScore: number, willingScore: number): TierResult {
  if (immersionScore >= 70 && willingScore >= 60)
    return {
      tier: "Aware & Ready",
      emoji: "🟠",
      badge: "HIGH IMMERSION · HIGH MOTIVATION",
      badgeColor: "#E67E22",
  ...
  if (immersionScore >= 70)
    return {
      tier: "Deep In It",
      emoji: "🔴",
      badge: "HIGH DEPENDENCY DETECTED",
      badgeColor: "#C0392B",
  ...
  if (immersionScore >= 40 && willingScore >= 55)
    return {
      tier: "In the Middle",
      emoji: "🟡",
      badge: "MODERATE USAGE · MOTIVATED TO SHIFT",
      badgeColor: "#D4AC0D",
  ...
  return {
    tier: "Relatively Present",
    emoji: "🟢",
    badge: "LOW DEPENDENCY · STRONG AWARENESS",
    badgeColor: "#2E7D5E",
...
async function postToAirtable(
  ...
  if (!AIRTABLE_API_KEY || AIRTABLE_API_KEY === "YOUR_AIRTABLE_API_KEY") {
    console.log(
      "[IRL] Airtable not configured — would have submitted:",
      fields
    );
    return true;
  }
...
function MultiInput({ qId, options, value = [], onChange }: MultiInputProps) {
  ...
  return (
    <div>
      ...
      >
        Select one or more, then continue scrolling
      </div>
    </div>
  );
}
...
function HighIntentBox({ immersionScore, tier, answers }: HighIntentBoxProps) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    setError("");
    const answersSummary = QUESTIONS.filter((q) => answers[q.id] !== undefined)
      .map((q) => {
    ...
    return (
      <div className="lead-box">
        <div className="lead-success">
          ...
          <p className="lead-success-sub">
            We'll be in touch soon. If you checked the call box, expect a
            message from us within a week.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="lead-box">
      ...
      <p className="lead-box-sub">
        We're recruiting 10 early testers for closed beta. No commitment — just
        a conversation, and early access before public launch.
      </p>
      <div className="lead-form">
        ...
        <label className="lead-checkbox-row">
          ...
          <span className="lead-checkbox-label">
            I'm open to a 15-minute call with the irl team — I want to share my
            experience and hear more about early access.
          </span>
        </label>
...
function LowIntentBox({ tier, immersionScore }: LowIntentBoxProps) {
  ...
  return (
    <div className="low-box">
      ...
      <p className="low-box-sub">
        irl is in development. We'll share updates, research, and early access
      ...
      {submitted ? (
        ...
        <div className="low-input-row">
          <input
            ...
            onChange={(e) => setEmail(e.target.value)}
          />
          <button className="low-submit" onClick={handleSubmit}>
            Notify me
          </button>
        </div>
      )}
...
function ResultScreen({ answers, onReset }: ResultScreenProps) {
  ...
  const { tier, emoji, badge, badgeColor, desc } = getTier(
    immersionScore,
    willingScore
  );
  ...
  const handleChange = useCallback(
    (qId: string, val: number | string | number[]) => {
      setAnswers((prev) => {
        ...
        if (qId === "q10" && typeof val === "number" && val < 5)
          delete next["q10a"];
        return next;
      });
    },
  ...
  return (
    <>
      ...
      <nav>
        ...
        <button className="nav-cta" onClick={scrollToSurvey}>
          Take the Survey
        </button>
      </nav>
      ...
      <section className="hero">
        ...
        <div className="hero-content">
          ...
          <p className="hero-sub">
            Blocking apps don't fix the root cause. irl learns why you scroll —
            and interrupts the loop before it starts.
          </p>
          <div className="hero-stats">
            <div className="stat">
              <span className="stat-num">10–16</span>
              <p className="stat-label">
                years of your remaining lifetime spent on your phone
              </p>
            </div>
            ...
            <div className="stat">
              ...
              <p className="stat-label">
                of phone users show signs of addiction
              </p>
            </div>
            <div className="divider-dot" />
            <div className="stat">
              ...
              <p className="stat-label">
                invested by platforms in helping you put the phone down
              </p>
            </div>
          </div>
          <button className="hero-cta" onClick={scrollToSurvey}>
      ...
      <section className="problem">
        <div className="problem-inner">
          <h2 className="problem-title">
            The tools you've tried weren't designed to help you.
          </h2>
          <p className="problem-body">
            App blockers create friction. They don't address{" "}
            <strong>why you open Instagram at 11pm</strong> when you're anxious,
            lonely, or avoiding something. irl is the first app that treats
            compulsive scrolling as a behavioral pattern — not a discipline
            problem.
          </p>
        </div>
      </section>
      ...
      <section className="how">
        ...
        <div className="how-grid">
          {[
            {
              n: "01",
              title: "irl learns your patterns",
              body: "The on-device AI observes when you scroll, for how long, and what preceded it — building a behavioral map unique to you.",
            },
            {
              n: "02",
              title: "Intervenes at the right moment",
              body: "At the moment you're about to open a distracting app, irl surfaces a personalized prompt — not a hard block, a gentle redirect.",
            },
            {
              n: "03",
              title: "Redirects you toward meaning",
              body: "You told irl what actually matters to you. It steers you there — friends, hobbies, rest — instead of the infinite feed.",
            },
          ].map((s) => (
      ...
      <section className="survey-section" id="survey-anchor">
        <div className="survey-inner">
          ...
          <p className="survey-sub">
            A short diagnostic on your relationship with technology.
            <br />
            Answer honestly. Your screen time stats already know.
          </p>
          {phase === "survey" ? (
      ...
      <section className="testimonials">
        ...
        <div className="testi-grid">
          {[
            {
              quote:
                '"Apple\'s features have no friction, and existing apps are too complex to set up."',
              who: "23M, University Student",
              screen: "5.6 hrs/day",
            },
            {
              quote:
                "\"I can't put my phone down, and it's hard to focus on my studies. I bought an app blocker for a lifetime but it just doesn't work for me.\"",
              who: "23M, University Student",
              screen: "13 hrs/day",
            },
            {
              quote:
                '"I go on my phone as a break, to decompress. I would like to learn new hobbies but I don\'t have the time."',
              who: "22F, University Student",
              screen: "4.5 hrs/day",
            },
          ].map((t, i) => (
      ...
      <footer>
        ...
        <p className="footer-fine">
          Built in Canada 🍁 · NEXT Canada Program · © 2025 irl
        </p>
      </footer>
    </>
  );
